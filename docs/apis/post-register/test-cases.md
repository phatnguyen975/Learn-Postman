...
| --------------- | ------------------------------------------------------------ | ------------- | --------------------------------------------------------------------------------------- | ----------------------------------------- | -------------- | ------ | ------------- |
| TC-FR01-SEC-001 | Verify response body does not expose password field          | Data exposure | name: "SecTest2", email: "sectest2@domain.com", password: "Password123!"                | 200 OK — no `password` in body            | SEC-01         | —      | —             |
| TC-FR01-SEC-002 | Verify SQL injection in email field does not cause data leak | SQLi          | name: "Sec Test", email: "' OR '1'='1", password: "Password123!"                        | 400 Bad Request — no side effect          | SEC-05         | —      | —             |
| TC-FR01-SEC-003 | Verify SQL injection in name field is blocked                | SQLi          | name: "'; SELECT * FROM users;--", email: "sqli2@domain.com", password: "Password123!"  | 200 OK or 400 — parameterized query       | SEC-05         | —      | —             |
| TC-FR01-SEC-004 | Verify error response does not expose raw SQL messages       | Misconfig     | Trigger DB error                                                                        | 500 — body must NOT contain SQL text      | Sensitive Data | —      | —             |
| TC-FR01-SEC-005 | Verify Content-Type header is application/json               | Misconfig     | Valid request                                                                           | 200 OK — `Content-Type: application/json` | API8:2023      | —      | —             |
...
| TC-FR01-SCH-003 | Verify success response has no extra fields        | Success   | body                   | `additionalProperties: false` | —      | —             |
| TC-FR01-SCH-004 | Verify success response Content-Type header        | Success   | `Content-Type`         | `application/json`            | —      | —             |
| TC-FR01-SCH-005 | Verify 400 error body has error field              | 400 Error | `error`                | string                        | —      | —             |
| TC-FR01-SCH-006 | Verify 409 error body has error field              | 409 Error | `error`                | string                        | —      | —             |
...
| TC-FR01-ERR-005 | Register account with name field as integer         | name: 12345, email: "int-name@domain.com", password: "Password123!"              | 400 Bad Request | Type error                      | —      | —             |
| TC-FR01-ERR-006 | Register account with email field as integer        | name: "Int Email", email: 12345, password: "Password123!"                        | 400 Bad Request | Type error                      | —      | —             |
| TC-FR01-ERR-007 | Register account with password field as boolean     | name: "Bool Pass", email: "bool-pass@domain.com", password: true                 | 400 Bad Request | Type error                      | —      | —             |
| TC-FR01-ERR-008 | Register account with name field missing            | email: "no-name@domain.com", password: "Password123!"                            | 400 Bad Request | Error: name is required         | —      | —             |
| TC-FR01-ERR-009 | Register account with email field missing           | name: "No Email", password: "Password123!"                                       | 400 Bad Request | Error: email is required        | —      | —             |
| TC-FR01-ERR-010 | Register account with password field missing        | name: "No Pass", email: "no-pass@domain.com"                                     | 400 Bad Request | Error: password is required     | —      | —             |
| TC-FR01-ERR-011 | Register account with all fields set to null        | name: null, email: null, password: null                                          | 400 Bad Request | Error: all required             | —      | —             |
| TC-FR01-ERR-012 | Register account with unknown extra fields          | name: "Extra", email: "extra@domain.com", password: "Password123!", extra: "val" | 200 or 400      | Extra field ignored or rejected | —      | —             |
